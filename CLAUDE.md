...
| `netlify/functions/lib/recommend.js` | `makeFirstRecommendations` (onboarding one-shot: COMPLEMENTARY match → semantic fallback → top 3 → matchLog per candidate → complementarity-aware blurb) + `findComplementaryMatches` (needs↔offers bidirectional match) + `runConnectorSearch` (connector-mode conversational search: NL query → real matches → matchLog + 2nd-pass intro blurb) |
